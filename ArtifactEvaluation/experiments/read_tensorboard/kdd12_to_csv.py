...
for i in range(5):
    for c in range(method_cr[i]):
        ...
        print(len(loss))
        print(loss)
        pd.DataFrame(ea.Scalars('Train/Loss')).to_csv('./excels/kdd12/' + str(md) + str(cr) + '_loss.csv')
        lst = 0
        avr_loss = 0
        avr_auc = auc[-2].value
        for x in loss:
        ...
        pd.DataFrame(ea.Scalars('roc_auc')[:-1]).to_csv('./excels/kdd12/' + str(md) + str(cr) + '_auc.csv')
        auc_csv.loc[cr, md] = ea.Scalars('roc_auc')[-2].value
        loss_csv.loc(cr, md) = avr_loss

auc_csv.to_csv("./excels/kdd12/auc.csv")
loss_csv.to_csv("./excels/kdd12/loss.csv")