...
def bar_throughput():
    ...
    plt.bar(x, data['Hash'] / 1e3, width=width, color='C0',
            label='Hash', edgecolor='black')
    plt.bar(x + width, data['Q-R Trick'] / 1e3, width=width, color='C1',
            label='Q-R Trick', edgecolor='black')
    plt.bar(x + 2*width, data['MDE'] / 1e3, width=width, color='C5',
            label='MDE', edgecolor='black')
    plt.bar(x + 3*width, data['AdaEmbed'] / 1e3, width=width, color='C2',
            label='AdaEmbed', edgecolor='black')  # ada freq
    plt.bar(x + 4*width, data['CAFE(ours)'] / 1e3, width=width, color='C3',
