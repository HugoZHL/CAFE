...
def wdl_wrap(X, lS_o, lS_i, use_gpu, device, ndevices=1, test=False, sk_flag=False):
    with record_function("DLRM forward"):
        if use_gpu:  # .cuda()
            ...
            if ndevices == 1:
                if sk_flag:
                    if X != None:
                        return wdl(X.to(device), lS_o, lS_i, test)
                    else:
                        return wdl(None, lS_o, lS_i, test)
                else:
                    lS_i = (
...
def loss_fn_wrap(Z, T, use_gpu, device):
    with record_function("DLRM loss compute"):
        if args.loss_function == "mse" or args.loss_function == "bce":
            return wdl.loss_fn(Z, T.to(device))
        elif args.loss_function == "wbce":
            loss_ws_ = wdl.loss_ws[T.data.view(-1).long()
                                    ].view_as(T).to(device)
            loss_fn_ = wdl.loss_fn(Z, T.to(device))
...
### define wdl in PyTorch ###
class WDL_Net(nn.Module):
        ...
        if (
            ...
            if ndevices <= 1:
                ...
                if self.weighted_pooling == "learned":
                    ...
                    self.v_W_l = w_list
            m_den = 13
            ln_top = np.array([m_spa * len(ln_emb) + m_den,256,256,1])
    ...
    def insert_adagrad(self, lS_o):
        ...
        for k, input in enumerate(lS_o):
            if self.ada_emb[k] == True:
                ...
                tmp += N
        self.d_time += 1
        if (self.d_time == 1 or self.d_time % 8192 == 0):
            self.ada_check()
...
#        table_feature_map = {idx: idx for idx in range(len(train_data.counts))}
    ...
    if use_gpu:
        ...
        wdl = wdl.to(device)  # .cuda()
        if wdl.ndevices > 1:
            wdl.emb_l, wdl.v_W_l = wdl.create_emb(
                m_spa, ln_emb, args.weighted_pooling
            )
        else:
            if wdl.weighted_pooling == "fixed":
                for k, w in enumerate(wdl.v_W_l):
                    wdl.v_W_l[k] = w.cuda()
    ...
    if not (args.load_model == ""):
        ...
        wdl.load_state_dict(ld_model["state_dict"])
        if args.sketch_flag:
            wdl.lib.update()
        ld_j = ld_model["iter"]
        ld_k = ld_model["epoch"]
    ...
    if args.inference_only:
        ...
        if args.quantize_mlp_with_bit != 32:
            ...
            wdl = torch.quantization.quantize_dynamic(
                wdl, {torch.nn.Linear}, quantize_dtype
            )
        if args.quantize_emb_with_bit != 32:
            wdl.quantize_embedding(args.quantize_emb_with_bit)
            # print(dlrm)
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    with record_function("DLRM backward"):
                        ...
                        # if args.sketch_flag:
                        #     wdl.insert_grad(lS_i)
                        if args.ada_flag:
                            wdl.insert_adagrad(lS_i)
                        # optimizer
                        optimizer.step()
                        lr_scheduler.step()
